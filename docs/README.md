...
- **New to the project?** Read the [High-Level Design](./high-level-design.md), then walk [Tracing a Slice: Applying to a Job](./tracing-a-slice-apply-to-a-job.md) to see one real request move through every layer before skimming the [ADRs](./adr/README.md).
